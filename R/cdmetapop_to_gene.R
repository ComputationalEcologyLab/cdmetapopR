...
#' @param format A character string indicating the desired output format: "genepop" or "genalex". Genepop is default. 
#'
...
tmp<-adegenet::df2genind(new, sep = ":", pop = pop)
  ...
  if (format == "genepop") {
    graph4lg::genind_to_genepop(tmp, output = paste0("my_genepop_", tools::file_path_sans_ext(basename(path)), ".txt", sep = ""))
    message("GENEPOP file written")
  }
